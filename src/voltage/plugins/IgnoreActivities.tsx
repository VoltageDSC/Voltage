...
export default definePlugin({
    name: "Ignore Activities",
    authors: [Devs.Zach],
    description: "Ignore certain activities from showing up on your status. You can configure which ones are ignored from the Registered Games and Activities tabs.",
    patches: [{
        find: ".Messages.SETTINGS_GAMES_TOGGLE_OVERLAY",
        replacement: {
            match: /var .=(?<props>.)\.overlay.+?"aria-label":.\..\.Messages\.SETTINGS_GAMES_TOGGLE_OVERLAY.+?}}\)/,
            replace: "$&,Voltage.Plugins.plugins[\"Ignore Activities\"].renderToggleGameActivityButton($<props>)"
        }
    }, {
        find: ".overlayBadge",
        replacement: {
            match: /.badgeContainer.+?.\?\(0,.\.jsx\)\(.{1,2},{name:(?<props>.)\.name}\):null/,
            replace: "$&,Voltage.Plugins.plugins[\"Ignore Activities\"].renderToggleActivityButton($<props>)"
        }
    }, {
        find: '.displayName="LocalActivityStore"',
        replacement: {
            match: /(?<activities>.)\.push\(.\({type:.\..{1,3}\.LISTENING.+?\)\)/,
            replace: "$&;$<activities>=$<activities>.filter(Voltage.Plugins.plugins[\"Ignore Activities\"].isActivityNotIgnored);"
        }
    }],
