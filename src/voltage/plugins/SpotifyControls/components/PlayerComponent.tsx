...
const cl = (className: string) => `voltage-spotify-${className}`;
...
const useStateFromStores: <T>(stores: typeof SpotifyStore[], mapper: () => T, idk?: null, compare?: (old: T, newer: T) => boolean) => T = findByCodeLazy("useStateFromStores");

function Svg(path: string, label: string) {
    return () => (
        <svg className={classes(cl("button-icon"), cl(label))} height="24" width="24" viewBox="0 0 24 24" fill="currentColor" aria-label={label} focusable={false}>
            <path d={path} />
        </svg>
    );
}
...
function SeekBar() {
    ...
    return (
        <div className={cl("progress-bar")}>
            ...
            <Slider minValue={0} maxValue={duration} value={position} onChange={(v: number) => { if (isSettingPosition) return; setPosition(v); seek(v); }} renderValue={msToHuman} />
...
function AlbumContextMenu({ track }: { track: Track; }) {
    ...
    return (
        <Menu.ContextMenu navId="spotify-album-menu" onClose={() => FluxDispatcher.dispatch({ type: "CONTEXT_MENU_CLOSE" })} aria-label="Spotify Album Menu">
            <Menu.MenuItem key="open-album" id="open-album" label="Open Album" action={() => SpotifyStore.openExternal(`/album/${track.album.id}`)} />
            <Menu.MenuItem key="view-cover" id="view-cover" label="View Album Cover" action={() => (Voltage.Plugins.plugins["View Icons"] as any).openImage(track.album.image.url)} />
            <Menu.MenuControlItem id="spotify-volume" key="spotify-volume" label="Volume" control={(props, ref) => (<Slider {...props} ref={ref} value={volume} minValue={0} maxValue={100} onChange={debounce((v: number) => SpotifyStore.setVolume(v))} />)} />
        </Menu.ContextMenu>
    );
}
...
function Info({ track }: { track: Track; }) {
    ...
    const i = (
        <>
            {img && (
                <img className={cl("album-image")} src={img.url} alt="Album Image" onClick={() => setCoverExpanded(!coverExpanded)} onContextMenu={e => { ContextMenu.open(e, () => <AlbumContextMenu track={track} />); }} />
            )}
        </>
    );

    if (coverExpanded && img) return (
        <div className={cl("album-expanded-wrapper")}>
            {i}
        </div>
    );
    ...
    return (
        <div className={cl("info-wrapper")}>
            ...
            <div className={cl("titles")}>
                <Forms.FormText variant="text-sm/semibold" id={cl("song-title")} className={cl("ellipoverflow")} role={track.id ? "link" : undefined} title={track.name} onClick={track.id ? () => { SpotifyStore.openExternal(`/track/${track.id}`); } : void 0}>
                    {track.name}
                </Forms.FormText>
                    ...
                    <Forms.FormText variant="text-sm/normal" className={cl("ellipoverflow")}>
                        ...
                        {track.artists.map((a, i) => (
                            <React.Fragment key={a.name}>
                                <Link className={cl("artist")} disabled={!a.id} href={`https://open.spotify.com/artist/${a.id}`} style={{ fontSize: "inherit" }} title={a.name}>
                                    {a.name}
                                </Link>
                ...
                {track.album.name && (
                    <Forms.FormText variant="text-sm/normal" className={cl("ellipoverflow")}>
                        on&nbsp;
                        <Link id={cl("album-title")} href={`https://open.spotify.com/album/${track.album.id}`} target="_blank" className={cl("album")} disabled={!track.album.id} style={{ fontSize: "inherit" }} title={track.album.name}>
                            {track.album.name}
                        </Link>
                    </Forms.FormText>
                )}
