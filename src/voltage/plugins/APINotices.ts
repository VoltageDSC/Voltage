            ...
            replacement: [
                {
                    ...
                    replace: ";if(Voltage.Api.Notices.currentNotice)return false"
                },
                {
                    match: /(?<=,NOTICE_DISMISS:function\(\i\){)(?=if\(null==(\i)\))/,
                    replace: (_, notice) => `if(${notice}.id=="VoltageNotice")return(${notice}=null,Voltage.Api.Notices.nextNotice(),true);`
                }
            ]
        }
    ],
